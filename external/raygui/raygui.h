...
*           Draw control bounds rlrectangles for debug
*
*       #define RAYGUI_DEBUG_TEXT_BOUNDS
*           Draw text bounds rlrectangles for debug
*
*   VERSIONS HISTORY:
...
*       2.8 (03-May-2020) Centralized rlrectangles drawing to GuiDrawRectangle()
...
// Check if two rlrectangles are equal, used to validate a slider bounds as an id
...
static rlRectangle guiSliderActive = { 0 };       // Gui slider active bounds rlrectangle, used as an unique identifier
...
static void DrawRectangleGradientV(int posX, int posY, int width, int height, Color color1, Color color2);  // Draw rlrectangle vertical gradient
//-------------------------------------------------------------------------------
...
static void GuiDrawRectangle(rlRectangle rec, int borderWidth, Color borderColor, Color color);   // Gui draw rlrectangle using default raygui style
...
int GuiScrollPanel(rlRectangle bounds, const char *text, rlRectangle content, Vector2 *scroll, rlRectangle *view)
{
    ...
    *view = (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)?
    ...
    // Draw detail corner rlrectangle if both scroll bars are visible
...
int GuiDropdownBox(rlRectangle bounds, const char *text, int *active, bool editMode)
{
    ...
    if ((state != STATE_DISABLED) && (editMode || !guiLocked) && (itemCount > 1) && !guiSliderDragging)
    {
        ...
        if (editMode)
        {
            ...
            for (int i = 0; i < itemCount; i++)
            {
                // Update item rlrectangle y position for next item
...
// Dummy rlrectangle control, intended for placeholding
...
int GuiListViewEx(rlRectangle bounds, const char **text, int count, int *scrollIndex, int *active, int *focus)
{
    ...
    // Define base item rlrectangle [0]
...
void GuiLoadStyleDefault(void)
{
    ...
    if (guiFont.texture.id != GetFontDefault().texture.id)
    {
        ...
        // NOTE: We set up a 1px padding on char rlrectangle to avoid pixel bleeding on MSAA filtering
...
// Draw selected icon using rlrectangles pixel-by-pixel
...
static void GuiLoadStyleFromMemory(const unsigned char *fileData, int dataSize)
{
        ...
        (signature[3] == ' '))
    {
        ...
        if (fontDataSize > 0)
        {
            ...
            // Set font texture source rlrectangle to be used as white texture to draw shapes
...
static int GuiScrollBar(rlRectangle bounds, int value, int minValue, int maxValue)
{
    ...
    // Calculate rlrectangles for all of the components
...
// Check if point is inside rlrectangle
...
// Draw rlrectangle with vertical gradient fill color
...
#define DARK_STYLE_FONT_ATLAS_COMP_SIZE 2404

// Font atlas image pixels data: DEFLATE compressed
static unsigned char darkFontData[DARK_STYLE_FONT_ATLAS_COMP_SIZE] = { 0xec,
    ...
    0xf0, 0xeb, 0xea, 0xff, 0x3f, 0xed, 0xc6, 0x31, 0x09, 0x00, 0x00, 0x08, 0x00, 0x30, 0xc1, 0xdb, 0xb0, 0xf6, 0xf7, 0x10,
    0xc1, 0x18, 0xdb, 0xb5, 0x5b, 0xd7, 0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static void GuiLoadStyleDark(void)
{
    ...
    // TODO: font loading crashes because deflate algo has issues
